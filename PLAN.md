...
**`CATALOG.md` says 24 datasets, and both numbers are right.** `migrated` counts datasets whose *consumers* read this repo; the catalog counts datasets that *live* here. The six `high_dim_data` files landed 2026-08-10 at `status: landed` with `consumers: []`, so they are published and documented but nothing reads them yet — they close the gap only when PR set C repoints them. Expect the same six-file gap between the two figures for the duration of any wave that lands ahead of its repoints, which is the convention here.
...
`sources/` exists as of [#63](https://github.com/QuantEcon/data-lectures/pull/63). Its defining property is **un-refetchability, not size** — all six `committed` builders here fetch from their third-party upstream at run time and none has a committed input, so this is an exception layer rather than a general input tree. Confirmed not served: both `sources/` paths return 404 on Pages.
