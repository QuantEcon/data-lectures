#!/usr/bin/env python3
"""
Builder for lectures/hansen_singleton_1982_data.csv.

Constructs the monthly gross real market return and gross consumption-growth
series the `hansen_singleton_1982` lecture estimates on, from FRED and the Ken
French data library, over Hansen-Singleton's 1959:2-1978:12 sample.

The lecture's ND+VWR specification wants CRSP's value-weighted NYSE return and
BEA per-capita nondurables consumption. CRSP is not open access, so the market
leg uses the Ken French research factors (`Mkt-RF + RF`) as the closest public
proxy -- which is why the lecture matches the paper qualitatively rather than
exactly. Consumption is FRED's real nondurables index divided by the 16+
civilian noninstitutional population; the nominal return is deflated by the
month-over-month gross inflation of the nondurables price deflator.

`hansen_singleton_1983_data.csv` is built the same way and its columns are a
strict superset of this file's, bitwise -- see that builder and the two
manifests.

Migrated from QuantEcon/lecture-python.myst, where it lived beside its output
at lectures/_static/lecture_specific/hansen_singleton_1982/make_data.py. Both
...
OUT_FILE = 'hansen_singleton_1982_data.csv'

COLUMNS = ["gross_real_return", "gross_cons_growth"]
...
def validate(frame):
    ...
    # Both series are GROSS -- ratios near 1, not percentages and not net
    # returns. A units change upstream would pass every structural check above
    # and quietly rescale every figure in the lecture. Bands are wide relative
    # to the observed 1959-1978 spread (returns 0.87-1.16, growth 0.97-1.03).
